...
//read inputs buttons and accelerometer
GameInputs read_inputs()                  //Fixed this
{   
    GameInputs in;
    static bool b1_prev = 1;
    static bool b2_prev = 1;
    static bool b3_prev = 1;
    
    acc.readXYZGravity(&in.ax, &in.ay, &in.az);

    bool b1 = button1.read();
    bool b2 = button2.read();
    bool b3 = button3.read();
    in.b1   = !b1 && b1_prev;
    in.b2   = !b2 && b2_prev;
    in.b3   = !b3 && b3_prev;
    b1_prev = b1;
